...
function getPizza() {
  // get id of pizza
  // it doesn;t go over the use of this URLSearchParams, but it seems to grab values from the url
  ...
  fetch(`/api/pizzas/${pizzaId}`)
    ...
    .then(printPizza)
    // added a catch sot aht any error takes the user back to the homepage
    .catch(err => {
      console.log(err);
      alert('Cannot find a pizza with this id! Taking you back.');
      // the window history API exposes methods that let us control the state of the browser's session. As long as this particular browser session has a previous page, it will behave as if the user had clickd on the 'back' button.
      window.history.back();
    });
}
